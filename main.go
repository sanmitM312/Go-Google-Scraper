...
// return a random user agent from userAgents
...
// buildGoogleUrls builds google queries
func buildGoogleUrls(searchTerm string, countryCode string, languageCode string, pages int, count int) ([]string, error) {
	...
	if googleBase, found := googleDomains[countryCode]; found {
		for i := 0; i < pages; i++ {
			start := i * count
			scrapeURL := fmt.Sprintf("%s%s&num=%d&hl=%s&start=%d&filter=0", googleBase, searchTerm, languageCode, count, start)
...
// only used if you use a proxy string
...
func GoogleScrape(searchTerm, countryCode, languageCode string, proxyString interface{}, pages, count, backoff int) ([]SearchResult, error) {
	...
	for _, page := range googlePages {
		...
		// make res to []SearchResult type structure
		...
		// add the parsed data to our results
...
func scrapeClientRequest(searchURL string, proxyString interface{}) (*http.Response, error) {
	...
	if res.StatusCode != 200 {
		err := fmt.Errorf("scraper receive a non-200 status code suggesting a ban")
		return nil, err
	}
...
func main() {
	res, err := GoogleScrape("koustav dhar", "com", "en", nil, 1, 100, 10)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
